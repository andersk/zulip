...
v1_api_and_json_patterns = [
    ...
    path("realm/emoji", rest_dispatch, {"GET": "zerver.views.realm_emoji.list_emoji"}),
    re_path(
        r"^realm/emoji/(?P<emoji_name>.*)$",
        rest_dispatch,
        {
    ...
    path(
        "users/me/subscriptions/<int:stream_id>",
        rest_dispatch,
    ...
    path(
        ...
        rest_dispatch,
        {"POST": "zerver.views.reactions.add_reaction", "DELETE": "zerver.views.reactions.remove_reaction"},
    ),
    ...
    path(
        ...
        {
            "POST": (
                "zerver.views.user_settings.change_enter_sends",
    ...
    path(
        ...
        {
            "POST": (
                "zerver.views.hotspots.mark_hotspot_as_read",
    ...
    path(
        ...
        {
            "POST": (
                "zerver.views.tutorial.set_tutorial_status",
    ...
    path(
        "default_stream_groups/<int:group_id>/streams",
        rest_dispatch,
...
i18n_urls = [
    ...
    re_path(
        r"^accounts/new/send_confirm/(?P<email>[\S]+)?$",
        TemplateView.as_view(template_name="zerver/accounts_send_confirm.html"),
        {"realm_creation": True},
    ...
    re_path(
        r"^accounts/unsubscribe/(?P<email_type>[\w]+)/(?P<confirmation_key>[\w]+)$",
    ...
    path(
        ...
        zerver.views.home.accounts_accept_terms,
    ...
    path(
        "archive/streams/<int:stream_id>/topics/<str:topic_name>",
        zerver.views.archive.archive,
        name="zerver.views.archive.archive",
    ),
    path(
        "archive/streams/<int:stream_id>/topics",
    ...
    path(
        "integrations/doc-html/<str:integration_name>",
...
urls += [
    re_path(
        r"^user_uploads/temporary/([0-9A-Za-z]+)/([^/]+)$",
    ...
    re_path(
        r"^avatar/(?P<email_or_id>[\S]+)/(?P<medium>[\S]+)?$",
        rest_dispatch,
        {"GET": ("zerver.views.users.avatar", {"override_api_url_scheme"})},
    ),
    re_path(
        r"^avatar/(?P<email_or_id>[\S]+)$",
        rest_dispatch,
        {"GET": ("zerver.views.users.avatar", {"override_api_url_scheme"})},
    ),
]
...
urls += [
    path(
        ...
        zerver.views.report.report_csp_violations,
...
urls += [
    re_path(
        r"^external_content/(?P<digest>[\S]+)/(?P<received_url>[\S]+)$",
...
urls += [
    path(
        ...
        zerver.views.email_mirror.email_mirror_message,
