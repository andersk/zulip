    ...
    def by_group_pm_with(
        ...
        self_recipient_ids = [
            ...
            for recipient_tuple in Subscription.objects.filter(
                user_profile=self.user_profile, recipient__type=Recipient.HUDDLE,
            ).values("recipient_id")
        ]
        narrow_recipient_ids = [
            recipient_tuple["recipient_id"]
            for recipient_tuple in Subscription.objects.filter(
                user_profile=narrow_profile, recipient__type=Recipient.HUDDLE,
            ).values("recipient_id")
        ]
    ...
    def _by_search_tsearch(
        ...
        for term in re.findall(r'"[^"]+"|\S+', operand):
            if term[0] == '"' and term[-1] == '"':
                ...
                cond = or_(column("content").ilike(term), topic_column_sa().ilike(term))
...
def get_base_query_for_search(
    user_profile: UserProfile, need_message: bool, need_user_message: bool,
