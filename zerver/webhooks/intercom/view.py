...
def get_conversation_admin_single_created_message(payload: Dict[str, Any]) -> Tuple[str, str]:
    ...
    body = CONVERSATION_ADMIN_INITIATED_CONVERSATION.format(admin_name=assignee.get("name"), content=content)
    topic = get_topic_for_contacts(user)
    return (topic, body)
...
def get_user_tagged_message(payload: Dict[str, Any], action: str) -> Tuple[str, str]:
    user = payload["data"]["item"]["user"]
