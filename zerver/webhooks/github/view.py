...
def get_issue_body(payload: Dict[str, Any], include_title: bool = False) -> str:
    action = payload["action"]
    issue = payload["issue"]
...
def get_issue_comment_body(payload: Dict[str, Any], include_title: bool = False) -> str:
    action = payload["action"]
...
def get_status_body(payload: Dict[str, Any]) -> str:
    if payload["target_url"]:
        status = "[{}]({})".format(payload["state"], payload["target_url"])
    else:
        status = payload["state"]
    return "[{}]({}) changed its status to {}.".format(
        payload["sha"][:7], payload["commit"]["html_url"], status,  # TODO
    )
