...
* commit ([0a38cad](https://try.gitea.io/kostekIV/test/commit/0a38cad3fac3a13bb78b738d13f15ce9cc3343fa))"""
    ...
    def test_pull_request_edited(self) -> None:
        ...
        expected_message = (
            """kostekIV edited [PR #5](https://try.gitea.io/kostekIV/test/pulls/5) from `d` to `master`."""
        )
    ...
    def test_pull_request_reopened(self) -> None:
        expected_topic = "test / PR #1906 test 2"
        expected_message = (
            """kostekIV reopened [PR #5](https://try.gitea.io/kostekIV/test/pulls/5) from `d` to `master`."""
        )
    ...
    def test_pull_request_closed(self) -> None:
        expected_topic = "test / PR #1906 test 2"
        expected_message = (
            """kostekIV closed [PR #5](https://try.gitea.io/kostekIV/test/pulls/5) from `d` to `master`."""
        )
    ...
    def test_push_filtered_by_branches_ignore(self, check_send_webhook_message_mock: MagicMock) -> None:
        ...
        result = self.client_post(self.url, payload, HTTP_X_GITEA_EVENT="push", content_type="application/json")
