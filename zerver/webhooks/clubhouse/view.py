...
STORY_ESTIMATE_TEMPLATE = (
    "The estimate for the story {story_name_template} was set to {estimate}."
)
...
def get_update_archived_body(payload: Dict[str, Any], entity: str) -> str:
    ...
    kwargs = {
        "entity": entity,
        "name_template": get_name_template(entity).format(
            name=primary_action["name"], app_url=primary_action.get("app_url"),
        ),
        "action": action,
    }
...
def get_story_update_attachment_body(payload: Dict[str, Any]) -> Optional[str]:
    ...
    for ref in payload["references"]:
        if ref["id"] == file_id:
            kwargs.update({"type": ref["entity_type"], "file_name": ref["name"]})
