...
class TypesPrintTest(ZulipTestCase):
    ...
    def test_basic(self) -> None:
        self.check_signature("add(float, int) -> float", 5.0, add, 2.0, 3)
        self.check_signature("add(float, y=int) -> float", 5.0, add, 2.0, y=3)
    ...
    def test_dict(self) -> None:
        ...
        self.check_signature(
            "to_dict([(int, str), ...]) -> {int: str, ...}", {1: "one", 2: "two"}, to_dict, [(1, "one"), (2, "two")],
        )
    ...
    def test_sequence(self) -> None:
        ...
        self.check_signature("add(A([int]), B([str])) -> [int, ...]", [2, "two"], add, A([2]), B(["two"]))
        self.check_signature("add(A([int, ...]), y=B([])) -> [int, ...]", [2, "two"], add, A([2, "two"]), y=B([]))
