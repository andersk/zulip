...
class RedirectAndLogIntoSubdomainTestCase(ZulipTestCase):
    def test_data(self) -> None:
        ...
        response = redirect_and_log_into_subdomain(
            ExternalAuthResult(data_dict=data_dict),
        )
        data = load_subdomain_token(response)
        self.assertDictEqual(
            data,
...
class InviteUserTest(InviteUserBase):
    ...
    def test_invite_mirror_dummy_user(self) -> None:
        ...
        self.assertEqual(
            PreregistrationUser.objects.filter(email=mirror_user.email).count(), 0,
        )
        ...
        self.assertEqual(
            prereg_user.referred_by.email, inviter.email,
        )
...
so we didn't send them an invitation. We did send invitations to everyone else!",
    ...
    def test_confirmation_key_of_wrong_type(self) -> None:
        ...
        email_change_url = create_confirmation_link(
            prereg_user, Confirmation.EMAIL_CHANGE,
        )
...
class InvitationsTestCase(InviteUserBase):
    ...
    def test_successful_get_open_invitations(self) -> None:
        ...
        prereg_user_one.save()
        expired_datetime = timezone_now() - datetime.timedelta(
            days=(days_to_activate + 1),
        )
        prereg_user_two = PreregistrationUser(
            email="TestTwo@zulip.com", referred_by=user_profile,
        )
    ...
    def test_successful_member_resend_invitation(self) -> None:
        ...
        prereg_user_one.save()
        prereg_user = PreregistrationUser.objects.get(email=invitee)
        error_result = self.client_post(
            "/json/invites/" + str(prereg_user.id) + "/resend",
        )
...
class RealmCreationTest(ZulipTestCase):
    ...
    def check_able_to_create_realm(self, email: str, password: str = "test") -> None:
        ...
        self.assertEqual(result.status_code, 302)
        self.assertTrue(
            result["Location"].endswith(f"/accounts/new/send_confirm/{email}"),
        )
        result = self.client_get(result["Location"])
        self.assert_in_response("Check your email so we can get started.", result)
...
class UserSignUpTest(InviteUserBase):
    ...
    def test_bad_email_configuration_for_accounts_home(self) -> None:
        ...
        self.assertEqual(
            err.call_args_list[0][0], ("Error in accounts_home: %s", "uh oh"),
        )
    ...
    def test_bad_email_configuration_for_create_realm(self) -> None:
        ...
        self.assertEqual(
            err.call_args_list[0][0], ("Error in create_realm: %s", "uh oh"),
        )
    ...
    def test_signup_without_user_settings_from_another_realm(self) -> None:
        ...
        result = self.client_post(
            "/accounts/home/", {"email": email}, subdomain=subdomain,
        )
        self.assertEqual(result.status_code, 302)
    ...
    def test_failed_signup_due_to_nonexistent_realm(self) -> None:
        ...
        self.assertIn(
            "organization you are trying to join using {} does "
    ...
    def test_realm_redirect_with_next_param(self) -> None:
        ...
        self.assert_in_success_response(
            [
                ...
                'action="/accounts/go/?next=billing"',
            ],
            result,
        )
