...
class FetchInitialStateDataTest(ZulipTestCase):
    ...
    def test_delivery_email_presence_for_non_admins(self) -> None:
        ...
        do_set_realm_property(user_profile.realm, "email_address_visibility", Realm.EMAIL_ADDRESS_VISIBILITY_EVERYONE)
        result = fetch_initial_state_data(
            user_profile, None, "", client_gravatar=False, user_avatar_url_field_optional=False,
        )
...
class ClientDescriptorsTest(ZulipTestCase):
    ...
    def test_process_message_event_with_mocked_client_info(self) -> None:
        ...
        client_info = {
            ...
            "client:2": dict(client=client2, flags=["has_alert_word"]),
            "client:3": dict(client=client3, flags=[]),
            "client:4": dict(client=client4, flags=[]),
        }
