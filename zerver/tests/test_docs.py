...
class DocPageTest(ZulipTestCase):
    ...
    def test_integration_doc_endpoints(self) -> None:
        self._test(
            "/integrations/",
            "native integrations.",
            extra_strings=["And hundreds more through", "Hubot", "Zapier", "IFTTT"],
        )
...
class HelpTest(ZulipTestCase):
    ...
    def test_help_relative_links_for_stream(self) -> None:
        ...
        self.assertIn(
            '<a href="/#streams/subscribed">Your streams</a>', str(result.content),
        )
        self.assertEqual(result.status_code, 200)
...
class IntegrationTest(ZulipTestCase):
    ...
    def test_html_settings_links(self) -> None:
        ...
        self.assertEqual(
            context["settings_html"], '<a href="/#settings">Zulip settings page</a>',
        )
        self.assertEqual(
