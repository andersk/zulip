...
class HomeTest(ZulipTestCase):
    def test_home(self) -> None:
        ...
        self.assertEqual(set(result["Cache-Control"].split(", ")), {"must-revalidate", "no-store", "no-cache"})
    ...
    def test_num_queries_with_streams(self) -> None:
        ...
        for i in range(10):
            ...
            DefaultStream.objects.create(
                realm_id=realm_id, stream_id=stream.id,
            )
    ...
    def test_people(self) -> None:
        ...
        for i in range(3):
            defunct_user = self.create_non_active_user(
                realm=realm, email=f"defunct-{i}@zulip.com", name=f"Defunct User {i}",
            )
    ...
    def test_compute_navbar_logo_url(self) -> None:
        ...
        self.assertEqual(compute_navbar_logo_url(page_params), "/static/images/logo/zulip-org-logo.png?version=0")
    ...
    def test_furthest_read_time(self) -> None:
        ...
        hamlet = self.example_user("hamlet")
        self.login_user(hamlet)
        self.client_post("/json/messages/flags", {"messages": ujson.dumps([msg_id]), "op": "add", "flag": "read"})
