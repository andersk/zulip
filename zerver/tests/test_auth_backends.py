...
class AuthBackendTest(ZulipTestCase):
    ...
    def verify_backend(
        self, backend: Any, *, good_kwargs: Dict[str, Any], bad_kwargs: Optional[Dict[str, Any]] = None
    ) -> None:
        clear_supported_auth_backends_cache()
    ...
    def test_ldap_backend(self) -> None:
        ...
        self.verify_backend(
            backend,
            bad_kwargs=dict(request=mock.MagicMock(), username=username, password=password, realm=get_realm("zephyr")),
            good_kwargs=dict(request=mock.MagicMock(), username=username, password=password, realm=get_realm("zulip")),
        )
        self.verify_backend(
            backend,
            bad_kwargs=dict(request=mock.MagicMock(), username=username, password=password, realm=get_realm("zephyr")),
    ...
    def test_social_auth_backends(self) -> None:
        ...
        def patched_authenticate(request: Optional[HttpResponse] = None, **kwargs: Any) -> Any:
...
class SocialAuthBase(DesktopFlowTestingLib, ZulipTestCase):
    ...
    def social_auth_test_finish(
        self, result: HttpResponse, account_data_dict: Dict[str, str], expect_choose_email_screen: bool, **headers: Any
    ) -> HttpResponse:
    ...
    def test_social_auth_with_ldap_populate_registration_from_confirmation(self) -> None:
        ...
        with self.settings(
            ...
            self.stage_two_of_registration(
                result, realm, subdomain, email, name, "New LDAP fullname", skip_registration_form=True,
            )
...
class AppleAuthMixin:
    ...
    def generate_id_token(self, account_data_dict: Dict[str, str], audience: Optional[str] = None) -> str:
        ...
        id_token = jwt.encode(payload, private_key, algorithm="RS256", headers=headers).decode("utf-8")
...
class AppleAuthBackendNativeFlowTest(AppleAuthMixin, SocialAuthBase):
    ...
    def test_id_token_with_invalid_aud_sent(self) -> None:
        ...
        with self.apple_jwk_url_mock(), mock.patch("logging.info") as mock_info:
            result = self.client_get(url, **headers)
            mock_info.assert_called_once_with("/complete/apple/: %s", "Authentication failed: Token validation failed")
        return result
...
class GitHubAuthBackendTest(SocialAuthBase):
    ...
    def social_auth_test_finish(
        ...
        if expect_choose_email_screen:
            ...
            result = self.client_get(
                self.AUTH_FINISH_URL, dict(state=csrf_state, email=account_data_dict["email"]), **headers,
            )
    ...
    def test_social_auth_email_not_verified(self) -> None:
        ...
        with self.assertLogs(self.logger_string, level="WARNING") as m:
            result = self.social_auth_test(account_data_dict, subdomain="zulip", email_data=email_data)
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result.url, "/login/")
        self.assertEqual(
            m.output,
    ...
    def test_social_auth_github_team_not_member_failed(self) -> None:
        account_data_dict = self.get_account_data_dict(email=self.email, name=self.name)
        with mock.patch(
            "social_core.backends.github.GithubTeamOAuth2.user_data", side_effect=AuthFailed("Not found"),
        ), self.assertLogs(self.logger_string, level="INFO") as mock_info:
            result = self.social_auth_test(account_data_dict, subdomain="zulip")
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result.url, "/login/")
        self.assertEqual(mock_info.output, [self.logger_output("GitHub user is not member of required team", "info")])
...
class GoogleAuthBackendTest(SocialAuthBase):
    ...
    def get_log_into_subdomain(
        self, data: ExternalAuthDataDict, *, subdomain: str = "zulip", force_token: Optional[str] = None
    ) -> HttpResponse:
    ...
    def test_log_into_subdomain_when_using_invite_link(self) -> None:
        ...
        # Click confirm registration button.
        result = self.client_post(
            "/accounts/register/", {"full_name": "New User Name", "key": confirmation_key, "terms": True},
        )
        self.assertEqual(result.status_code, 302)
...
class FetchAPIKeyTest(ZulipTestCase):
    ...
    def test_wrong_password(self) -> None:
        result = self.client_post("/api/v1/fetch_api_key", dict(username=self.email, password="wrong"))
...
class TestZulipRemoteUserBackend(DesktopFlowTestingLib, ZulipTestCase):
    ...
    def test_login_desktop_flow_otp_success_email(self) -> None:
        ...
        # Verify that the right thing happens with an invalid-format OTP
        result = self.client_get("/accounts/login/sso/", dict(desktop_flow_otp="1234"), REMOTE_USER=email)
        self.assert_logged_in_user_id(None)
        self.assert_json_error_contains(result, "Invalid OTP", 400)
    ...
    def test_dev_ldap_fail_login(self) -> None:
        ...
        with self.settings(
            ...
            user_profile = self.backend.authenticate(
                request=mock.MagicMock(), username="ldapuser1", password="dapu", realm=get_realm("zulip"),
            )
    ...
    def test_email_and_ldap_backends_together(self) -> None:
        with self.settings(
            ...
            user_profile = EmailAuthBackend().authenticate(
                request=mock.MagicMock(), username=othello.delivery_email, password=password, realm=realm,
            )
...
class TestZulipLDAPUserPopulator(ZulipLDAPTestCase):
    ...
    def test_user_in_multiple_realms(self) -> None:
        ...
        hamlet2 = do_create_user(email, None, test_realm, hamlet.full_name, hamlet.short_name)
    ...
    def test_update_custom_profile_field_invalid_data(self) -> None:
        ...
        with self.settings(AUTH_LDAP_USER_ATTR_MAP={"full_name": "cn", "custom_profile_field__birthday": "birthDate"}):
...
class TestAdminSetBackends(ZulipTestCase):
    def test_change_enabled_backends(self) -> None:
        ...
        result = self.client_patch(
            "/json/realm", {"authentication_methods": ujson.dumps({"Email": False, "Dev": True})},
        )
