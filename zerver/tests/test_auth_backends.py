...
class AuthBackendTest(ZulipTestCase):
    def get_username(
        self, email_to_username: Optional[Callable[[str], str]] = None,
    ) -> str:
    ...
    def test_email_auth_backend(self) -> None:
        ...
        with mock.patch(
            "zproject.backends.email_auth_enabled", return_value=False,
    ...
    def test_login_preview(self) -> None:
        ...
        data = dict(
            description=ujson.dumps("New realm description"),
    ...
    def test_devauth_backend(self) -> None:
        self.verify_backend(
            DevAuthBackend(),
            good_kwargs=dict(
                dev_auth_username=self.get_username(), realm=get_realm("zulip"),
            ),
            bad_kwargs=dict(
                dev_auth_username=self.get_username(), realm=get_realm("zephyr"),
            ),
        )
    ...
    def test_social_auth_backends(self) -> None:
        ...
        def patched_authenticate(
            request: Optional[HttpResponse] = None, **kwargs: Any,
        ) -> Any:
        ...
        for backend_name in backends_to_test:
            with responses.RequestsMock(
                ...
                with mock.patch(
                    ...
                    self.verify_backend(
                        backend, good_kwargs=good_kwargs, bad_kwargs=bad_kwargs,
                    )
...
class RateLimitAuthenticationTests(ZulipTestCase):
    ...
    def do_test_auth_rate_limiting(
        self,
        attempt_authentication_func: Callable[
            [HttpRequest, str, str], Optional[UserProfile],
        ],
        username: str,
...
class DesktopFlowTestingLib(ZulipTestCase):
    ...
    def verify_desktop_data_and_return_key(
        self, desktop_data: str, desktop_flow_otp: str,
    ) -> str:
...
class SocialAuthBase(DesktopFlowTestingLib, ZulipTestCase):
    ...
    def social_auth_test_finish(
        ...
        **headers: Any,
        ...
        result = self.client_get(
            self.AUTH_FINISH_URL, dict(state=csrf_state), **headers,
        )
        return result

    def generate_access_url_payload(self, account_data_dict: Dict[str, str]) -> str:
        return json.dumps({"access_token": "foobar", "token_type": "bearer"})
    ...
    def social_auth_test(
        ...
        * user_agent: What user-agent to use for the HTTP requests.
        ...
        if result.status_code != 302 or not result.url.startswith(
        ...
        with responses.RequestsMock(
            ...
            self.register_extra_endpoints(
                requests_mock, account_data_dict, **extra_data,
            )
    ...
    def test_config_error_development(self) -> None:
        if hasattr(self, "CLIENT_KEY_SETTING") and hasattr(
            self, "CLIENT_SECRET_SETTING",
        ):
            with self.settings(**{self.CLIENT_KEY_SETTING: None}):
                ...
                self.assert_in_success_response(
                    [self.CLIENT_KEY_SETTING.lower()], result,
                )
                self.assert_in_success_response(
                    [self.CLIENT_SECRET_SETTING.lower()], result,
                )
                ...
                self.assert_not_in_success_response(
                    ["zproject/dev_settings.py"], result,
                )
                self.assert_not_in_success_response(["/etc/zulip/settings.py"], result)
                self.assert_not_in_success_response(
                    ["/etc/zulip/zulip-secrets.conf"], result,
                )
    ...
    def test_config_error_production(self) -> None:
        if hasattr(self, "CLIENT_KEY_SETTING") and hasattr(
            ...
            with self.settings(**{self.CLIENT_KEY_SETTING: None}):
                ...
                self.assert_not_in_success_response(
                    ["zproject/dev-secrets.conf"], result,
                )
    ...
    def test_social_auth_success(self) -> None:
        ...
        self.assertTrue(
            uri.startswith("http://zulip.testserver/accounts/login/subdomain/"),
        )
    ...
    def test_social_auth_mobile_success(self) -> None:
        ...
        account_data_dict = self.get_account_data_dict(
            email=self.email, name="Full Name",
        )
    ...
    @override_settings(TERMS_OF_SERVICE=None)
    def test_social_auth_with_ldap_populate_registration_from_confirmation(
        self,
    ) -> None:
        self.init_default_ldap_database()
        email = "newuser@zulip.com"
    ...
    def test_social_auth_complete(self) -> None:
        with mock.patch(
            ...
            result = self.client_get(
                reverse("social:complete", args=[self.backend.name]),
            )
            self.assertEqual(result.status_code, 302)
    ...
    def test_social_auth_invited_as_admin_but_expired(self) -> None:
        ...
        do_invite_users(
            iago, [email], [], invite_as=PreregistrationUser.INVITE_AS["REALM_ADMIN"],
        )
...
class SAMLAuthBackendTest(SocialAuthBase):
    ...
    def social_auth_test(
        ...
        **extra_data: Any,
        ...
        data = SAMLAuthBackend.get_data_from_redis(
            ujson.loads(relay_state)["state_token"],
        )
    ...
    def generate_saml_response(
        ...
        unencoded_saml_response = self.fixture_data(
            "samlresponse.txt", type="saml",
        ).format(
            email=email,
        ...
        with mock.patch.object(OneLogin_Saml2_Response, "is_valid", return_value=True):
            with mock.patch.object(
                ...
                relay_state = ujson.dumps(
                    dict(
                        state_token=SAMLAuthBackend.put_data_in_redis(
                            {"subdomain": "zulip"},
                        ),
                    ),
                )
            ...
            self.assertEqual(
                ...
                [
                    self.logger_output(
                        "AuthFailed: Authentication failed: SAML login failed: [] (None)",
                        "info",
                    ),
                ],
            )
    ...
    def test_social_auth_complete_bad_params(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                self.logger_output(
                    "/complete/saml/: No SAMLResponse in request.", "info",
                ),
            ],
        )
    ...
    def test_social_auth_complete_no_subdomain(self) -> None:
        with self.assertLogs(self.logger_string, level="INFO") as m:
            ...
            with mock.patch.object(
                SAMLAuthBackend, "choose_subdomain", return_value=None,
            ):
                result = self.client_post("/complete/saml/", post_params)
            self.assertEqual(result.status_code, 302)
    ...
    def test_social_auth_saml_bad_idp_param_on_login_page(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                self.logger_output(
                    "/login/saml/ : Bad idp param: KeyError: {}.".format("'idp'"),
                    "info",
                ),
            ],
        )
        ...
        self.assertEqual(
            ...
            [
                self.logger_output(
                    "/login/saml/ : Bad idp param: KeyError: {}.".format("'bad_idp'"),
                    "info",
                ),
            ],
        )
    ...
    def test_social_auth_saml_multiple_idps_configured(self) -> None:
        ...
        with self.settings(SOCIAL_AUTH_SAML_ENABLED_IDPS=idps_dict):
            ...
            self.assert_in_success_response(
                ["/accounts/login/social/saml/test_idp"], result,
            )
            self.assert_in_success_response(["Log in with Second Test IdP"], result)
            self.assert_in_success_response(
                ["/accounts/login/social/saml/test_idp2"], result,
            )
    ...
    def test_social_auth_saml_idp_limited_to_subdomains_attempt_wrong_realm(
        self,
    ) -> None:
        idps_dict = copy.deepcopy(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS)
        idps_dict["test_idp"]["limit_to_subdomains"] = ["zulip"]
    ...
    def test_social_auth_saml_require_limit_to_subdomains(self) -> None:
        ...
        with self.settings(
            ...
            with self.assertLogs(self.logger_string, level="ERROR") as m:
                ...
                SAMLAuthBackend()
            self.assertEqual(
                list(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS.keys()), ["test_idp2"],
            )
        self.assertEqual(
            m.output,
            [
    ...
    def test_idp_initiated_signin_subdomain_implicit(self) -> None:
        ...
        with mock.patch.object(OneLogin_Saml2_Response, "is_valid", return_value=True):
            # We're not able to generate valid signatures in tests, so we need the mock.
            result = self.client_post(
                "http://zulip.testserver/complete/saml/", post_params,
            )
    ...
    def test_idp_initiated_signin_subdomain_implicit_invalid(self) -> None:
        ...
        with self.assertLogs(self.logger_string, level="INFO") as m:
            with mock.patch("zproject.backends.get_subdomain", return_value="invalid"):
                ...
                result = self.client_post(
                    "http://invalid.testserver/complete/saml/", post_params,
                )
...
class AppleIdAuthBackendTest(AppleAuthMixin, SocialAuthBase):
    ...
    def social_auth_test_finish(
        ...
        **headers: Any,
        ...
        self.client.session.flush()
        result = self.client_post(self.AUTH_FINISH_URL, dict(state=state), **headers)
        return result
    ...
    def test_apple_auth_enabled(self) -> None:
        with self.settings(
            AUTHENTICATION_BACKENDS=("zproject.backends.AppleAuthBackend",),
        ):
    ...
    def test_id_token_verification_failure(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                self.logger_output(
                    "AuthFailed: Authentication failed: Token validation failed",
                    "info",
                ),
            ],
        )
    ...
    def test_validate_state(self) -> None:
        with self.assertLogs(self.logger_string, level="INFO") as m:
            ...
            fake_state = (
                "fa42e4ccdb630f0070c1daab70ad198d8786d4b639cd7a1b4db4d5a13c623060"
            )
...
class AppleAuthBackendNativeFlowTest(AppleAuthMixin, SocialAuthBase):
    ...
    def test_social_auth_session_fields_cleared_correctly(self) -> None:
        ...
        def initiate_auth(mobile_flow_otp: Optional[str] = None) -> None:
            url, headers = self.prepare_login_url_and_headers(
                subdomain="zulip", id_token="invalid", mobile_flow_otp=mobile_flow_otp,
            )
            result = self.client_get(url, **headers)
            self.assertEqual(result.status_code, 302)
    ...
    def test_id_token_with_invalid_aud_sent(self) -> None:
        ...
        url, headers = self.prepare_login_url_and_headers(
            ...
            id_token=self.generate_id_token(
                account_data_dict, audience="com.different.app",
            ),
        )
...
class GitHubAuthBackendTest(SocialAuthBase):
        ...
        **headers: Any,
        ...
        if expect_choose_email_screen:
            ...
            if all_emails_verified:
                self.assert_not_in_success_response(
                    ["also has unverified email"], result,
                )
            else:
    ...
    def test_github_unverified_email_with_existing_account(self) -> None:
        ...
        account_data_dict = self.get_account_data_dict(
            email="hamlet@zulip.com", name=self.name,
        )
        email_data = [
...
class GitLabAuthBackendTest(SocialAuthBase):
    ...
    def test_gitlab_auth_enabled(self) -> None:
        with self.settings(
            AUTHENTICATION_BACKENDS=("zproject.backends.GitLabAuthBackend",),
        ):
...
class GoogleAuthBackendTest(SocialAuthBase):
    ...
    def test_social_auth_mobile_realm_uri(self) -> None:
        ...
        with self.settings(
            REALM_MOBILE_REMAP_URIS={
                "http://zulip.testserver": "http://zulip-mobile.testserver",
            },
        ):
            result = self.social_auth_test(
                account_data_dict,
    ...
    def test_google_auth_enabled(self) -> None:
        with self.settings(
            AUTHENTICATION_BACKENDS=("zproject.backends.GoogleAuthBackend",),
        ):
    ...
    def test_redirect_to_next_url_for_log_into_subdomain(self) -> None:
        def test_redirect_to_next_url(next: str = "") -> HttpResponse:
            ...
            user_profile = self.example_user("hamlet")
            with mock.patch(
                "zerver.views.auth.authenticate", return_value=user_profile,
            ):
    ...
    def test_log_into_subdomain_when_token_is_malformed(self) -> None:
        ...
        with mock.patch("logging.warning") as mock_warn:
            ...
            mock_warn.assert_called_once_with(
                "log_into_subdomain: Malformed token given: %s", "nonsense",
            )
    ...
    def test_log_into_subdomain_when_is_signup_is_true_and_new_user(self) -> None:
        ...
        result = self.client_post(
            "/accounts/register/", confirmation_data, subdomain="zulip",
        )
        self.assert_in_response("We just need you to do one last thing", result)
...
class FetchAPIKeyTest(ZulipTestCase):
    ...
    def test_deactivated_realm(self) -> None:
        ...
        self.assert_json_error_contains(
            result, "This organization has been deactivated", 403,
        )
...
class DevFetchAPIKeyTest(ZulipTestCase):
    ...
    def test_success(self) -> None:
        result = self.client_post(
            "/api/v1/dev_fetch_api_key", dict(username=self.email),
        )
        self.assert_json_success(result)
        data = result.json()
        self.assertEqual(data["email"], self.email)
...
class ExternalMethodDictsTests(ZulipTestCase):
    ...
    def test_get_external_method_dicts_correctly_sorted(self) -> None:
        with self.settings(
            ...
            self.assertEqual(
                [
                    social_backend["name"]
                    for social_backend in external_auth_methods[1:]
                ],
                [
                    social_backend.name
    ...
    def test_get_external_method_buttons(self) -> None:
        with self.settings(
            ...
            result = self.client_get("/login/")
            self.assert_in_success_response(
                [string.format("login") for string in expected_button_id_strings],
                result,
            )

            result = self.client_get("/register/")
            self.assert_in_success_response(
                [string.format("register") for string in expected_button_id_strings],
                result,
            )
...
class FetchAuthBackends(ZulipTestCase):
    def test_get_server_settings(self) -> None:
        def check_result(
            result: HttpResponse,
            extra_fields: Sequence[Tuple[str, Validator[object]]] = [],
        ) -> None:
            authentication_methods_list = [
                ("password", check_bool),
            ]
            for backend_name_with_case in AUTH_BACKEND_NAME_MAP:
                authentication_methods_list.append(
            ...
            checker = check_dict_only(
                [
                    (
                        ...
                        check_dict_only(authentication_methods_list),
                    ),
                    (
        ...
        check_result(result)
        self.assertEqual(
            result.json()["external_authentication_methods"],
            get_external_method_dicts(),
        )
...
class TestDevAuthBackend(ZulipTestCase):
    ...
    def test_choose_realm(self) -> None:
        ...
        result = self.client_post(
            "/devlogin/", {"new_realm": "all_realms"}, subdomain="zephyr",
        )
        self.assertEqual(result.status_code, 200)
        self.assert_in_success_response(["starnine@mit.edu", "espuser@mit.edu"], result)
        self.assert_in_success_response(["Click on a user to log in!"], result)
    ...
    def test_choose_realm_with_subdomains_enabled(self) -> None:
        with mock.patch(
            "zerver.views.auth.is_subdomain_root_or_alias", return_value=False,
        ):
            with mock.patch(
                "zerver.views.auth.get_realm_from_request",
            ...
            with mock.patch(
                ...
                self.assert_in_success_response(
                    ["Click on a user to log in to MIT!"], result,
                )
    ...
    def test_login_failure(self) -> None:
        ...
        with self.settings(
            ...
            self.assertRedirects(
                response,
                reverse("config_error", kwargs={"error_category_name": "dev"}),
            )
    ...
    def test_redirect_to(self) -> None:
        ...
        def test_with_redirect_to_param_set_as_next(next: str = "") -> HttpResponse:
            ...
            with self.settings(
                ...
                result = self.client_get(
                    "/accounts/login/sso/?next=" + next, REMOTE_USER=email,
                )
            return result
        ...
        res = test_with_redirect_to_param_set_as_next(
            "https://rogue.zulip-like.server/login",
        )
        self.assertEqual("http://zulip.testserver", res.url)
...
class TestJWTLogin(ZulipTestCase):
    ...
    def test_login_success(self) -> None:
        ...
        with self.settings(
            JWT_AUTH_KEYS={"zulip": {"key": "key", "algorithms": ["HS256"]}},
        ):
            email = self.example_email("hamlet")
            realm = get_realm("zulip")
    ...
    def test_login_failure_due_to_wrong_subdomain(self) -> None:
        payload = {"user": "hamlet", "realm": "zulip.com"}
        with self.settings(
            JWT_AUTH_KEYS={"acme": {"key": "key", "algorithms": ["HS256"]}},
        ):
            with mock.patch(
                "zerver.views.auth.get_subdomain", return_value="acme",
    ...
    def test_login_failure_due_to_empty_subdomain(self) -> None:
        ...
        with self.settings(JWT_AUTH_KEYS={"": {"key": "key", "algorithms": ["HS256"]}}):
            with mock.patch(
                "zerver.views.auth.get_subdomain", return_value="",
            ), mock.patch("logging.warning"):
...
class DjangoToLDAPUsernameTests(ZulipTestCase):
    ...
    def test_django_to_ldap_username_with_append_domain(self) -> None:
        with self.settings(LDAP_APPEND_DOMAIN="zulip.com"):
            ...
            self.assertEqual(
                self.backend.django_to_ldap_username("hamlet@zulip.com"), "hamlet",
            )
    ...
    def test_django_to_ldap_username_with_email_search(self) -> None:
        self.assertEqual(
            self.backend.django_to_ldap_username("hamlet"),
...
class TestLDAP(ZulipLDAPTestCase):
    def test_generate_dev_ldap_dir(self) -> None:
        ...
        for key, value in ldap_dir.items():
            self.assertTrue(regex.match(key))
            self.assertCountEqual(
                list(value.keys()), common_attrs + ["uid", "jpegPhoto"],
            )
    ...
    def test_email_and_ldap_backends_together(self) -> None:
        with self.settings(
            ...
            realm = get_realm("zulip")
            self.assertEqual(
                email_belongs_to_ldap(realm, self.example_email("aaron")), True,
            )
            username = self.ldap_username("aaron")
            ...
            self.assertEqual(
                email_belongs_to_ldap(realm, othello.delivery_email), False,
            )
...
class TestZulipLDAPUserPopulator(ZulipLDAPTestCase):
    ...
    def test_ldaperror_doesnt_deactivate_user(
        self, mock_deactivate: mock.MagicMock,
    ) -> None:
        """
        ...
        with mock.patch.object(
            _LDAPUser, "_get_or_create_user", side_effect=ldap.LDAPError,
        ):
    ...
    def test_user_in_multiple_realms(self) -> None:
        ...
        hamlet2 = do_create_user(
            email, None, test_realm, hamlet.full_name, hamlet.short_name,
        )
    ...
    def test_update_user_avatar(self) -> None:
        ...
        self.change_ldap_user_attr(
            "hamlet",
            "jpegPhoto",
            static_path("images/logo/zulip-icon-512x512.png"),
            binary=True,
        )
        with mock.patch("zerver.lib.upload.upload_avatar_image") as fn, self.settings(
            AUTH_LDAP_USER_ATTR_MAP={"full_name": "cn", "avatar": "jpegPhoto"},
    ...
    def test_update_custom_profile_field(self) -> None:
        ...
        hamlet = self.example_user("hamlet")
        test_data = [
            {"field_name": "Phone number", "expected_value": "123456789"},
    ...
    def test_update_non_existent_profile_field(self) -> None:
        with self.settings(
            ...
            with self.assertRaisesRegex(
                ZulipLDAPException,
                "Custom profile field with name non_existent not found",
            ):
                self.perform_ldap_sync(self.example_user("hamlet"))
    ...
    def test_update_custom_profile_field_no_mapping(self) -> None:
        hamlet = self.example_user("hamlet")
        no_op_field = CustomProfileField.objects.get(
    ...
    def test_update_custom_profile_field_no_update(self) -> None:
        ...
        birthday_field = CustomProfileField.objects.get(
            realm=hamlet.realm, name="Birthday",
        )
        ...
        expected_call_args = [
            hamlet,
            [{"id": birthday_field.id, "value": "1900-09-08"}],
        ]
        with self.settings(
            AUTH_LDAP_USER_ATTR_MAP={
                "full_name": "cn",
...
class TestQueryLDAP(ZulipLDAPTestCase):
    ...
    def test_normal_query(self) -> None:
        ...
        self.assertIn(
            "custom_profile_field__phone_number: LDAP field not present", values,
        )
...
class TestAdminSetBackends(ZulipTestCase):
    ...
    def test_disable_all_backends(self) -> None:
        ...
        self.assert_json_error(
            result, "At least one authentication method must be enabled.",
        )
        realm = get_realm("zulip")
        self.assertTrue(password_auth_enabled(realm))
        self.assertTrue(dev_auth_enabled(realm))
...
class LDAPBackendTest(ZulipTestCase):
    ...
    def test_non_existing_realm(self) -> None:
        ...
        with mock.patch(
            "zproject.backends.ZulipLDAPAuthBackend.get_or_build_user",
            side_effect=error,
            ...
            self.assertEqual(
                response.url,
                reverse("config_error", kwargs={"error_category_name": "ldap"}),
            )
            response = self.client_get(response.url)
            self.assert_in_response(
                "You are trying to login using LDAP " "without creating an", response,
            )
