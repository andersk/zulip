...
class NormalActionsTest(BaseAction):
    ...
    def test_stream_send_message_events(self) -> None:
        ...
        events = self.verify_action(
            lambda: self.send_stream_message(
                self.example_user("hamlet"), "Verona", "hello",
            ),
    ...
    def test_invite_user_event(self) -> None:
        schema_checker = check_events_dict([("type", equals("invites_changed"))])
    ...
    def test_user_group_events(self) -> None:
        ...
        user_group_update_checker = check_events_dict(
            [
                ...
                ("op", equals("update")),
                ("group_id", check_int),
                ("data", check_dict_only([("name", check_string)])),
            ],
        )
        ...
        user_group_update_checker = check_events_dict(
            [
                ...
                ("op", equals("update")),
                ("group_id", check_int),
                ("data", check_dict_only([("description", check_string)])),
            ],
        )
    ...
    def test_realm_update_plan_type(self) -> None:
        ...
        schema_checker = check_events_dict(
            [
                ...
                ("extra_data", check_dict_only([("upload_quota", check_int)])),
            ],
        )
    ...
    def test_change_bot_default_sending_stream(self) -> None:
        ...
        events = self.verify_action(action)
        self.realm_bot_schema("default_sending_stream", check_string)(
            "events[0]", events[0],
        )

        action = lambda: do_change_default_sending_stream(bot, None)
        events = self.verify_action(action)
        self.realm_bot_schema("default_sending_stream", equals(None))(
            "events[0]", events[0],
        )
    ...
    def test_change_bot_owner(self) -> None:
        ...
        change_bot_owner_checker_bot = check_events_dict(
            [
                ...
                (
                    "bot",
                    check_dict_only([("user_id", check_int), ("owner_id", check_int)]),
                ),
            ],
        )
        self.user_profile = self.example_user("iago")
        ...
        change_bot_owner_checker_bot = check_events_dict(
            [
                ("type", equals("realm_bot")),
                ("op", equals("delete")),
                ("bot", check_dict_only([("user_id", check_int)])),
            ],
        )
    ...
    def test_add_attachment(self) -> None:
        ...
        schema_checker = check_events_dict(
            [
                ("type", equals("attachment")),
                ("op", equals("remove")),
                ("attachment", check_dict_only([("id", check_int)])),
...
class RealmPropertyActionTest(BaseAction):
    def do_set_realm_property_test(self, name: str) -> None:
        ...
        test_values: Dict[str, Any] = dict(
            ...
            video_chat_provider=[Realm.VIDEO_CHAT_PROVIDERS["jitsi_meet"]["id"]],
...
class SubscribeActionTest(BaseAction):
    ...
    def do_test_subscribe_events(self, include_subscribers: bool) -> None:
        ...
        subscription_schema_checker = check_list(check_dict_only(subscription_fields))
