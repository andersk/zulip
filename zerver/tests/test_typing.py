...
class TypingValidateOperatorTest(ZulipTestCase):
    def test_missing_parameter(self) -> None:
        ...
        params = dict(to=ujson.dumps([sender.id]))
    ...
    def test_invalid_parameter(self) -> None:
        ...
        """
        sender = self.example_user("hamlet")
        params = dict(to=ujson.dumps([sender.id]), op="foo")
        result = self.api_post(sender, "/api/v1/typing", params)
...
class TypingHappyPathTest(ZulipTestCase):
    def test_start_to_single_recipient(self) -> None:
        ...
        params = dict(to=ujson.dumps([recipient_user.id]), op="start")
    ...
    def test_start_to_another_user(self) -> None:
        ...
        params = dict(to=ujson.dumps([recipient.id]), op="start")
    ...
    def test_stop_to_self(self) -> None:
        ...
        events: List[Mapping[str, Any]] = []
        with tornado_redirected_to_list(events):
            params = dict(to=ujson.dumps([user.id]), op="stop")
    ...
    def test_stop_to_another_user(self) -> None:
        ...
        events: List[Mapping[str, Any]] = []
        with tornado_redirected_to_list(events):
            params = dict(to=ujson.dumps([recipient.id]), op="stop")
            result = self.api_post(sender, "/api/v1/typing", params)
