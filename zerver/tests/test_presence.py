...
class UserPresenceTests(ZulipTestCase):
    ...
    def test_set_idle(self) -> None:
        ...
        self.assertEqual(
            set(presences.keys()), {str(hamlet.id), str(othello.id)},
        )
        ...
        self.assertGreaterEqual(
            othello_info["idle_timestamp"], hamlet_info["idle_timestamp"],
        )
    ...
    def test_set_active(self) -> None:
        ...
        self.assertEqual(
            set(othello_info.keys()), {"active_timestamp"},
        )

        self.assertEqual(
            set(hamlet_info.keys()), {"active_timestamp"},
        )

        self.assertGreaterEqual(
            hamlet_info["active_timestamp"], othello_info["active_timestamp"],
        )
    ...
    def test_new_user_input(self, unused_mock: mock.Mock) -> None:
        ...
        with mock.patch("zerver.views.presence.timezone_now", return_value=time_zero):
            result = self.client_post(
                ...
                {"status": "active", "new_user_input": "true"},
            )
        self.assert_json_success(result)
        self.assertEqual(
    ...
    def test_same_realm(self) -> None:
        ...
        self.assertEqual(
            json["presences"].keys(), {hamlet.email},
        )
...
class SingleUserPresenceTests(ZulipTestCase):
    ...
    def test_ping_only(self) -> None:
        ...
        self.login("othello")
        req = dict(status="active", ping_only="true")
...
class GetRealmStatusesTest(ZulipTestCase):
    def test_get_statuses(self) -> None:
        ...
        json = result.json()
        self.assertEqual(
            set(json["presences"].keys()), {str(hamlet.id), str(othello.id)},
        )
