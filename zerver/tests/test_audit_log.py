...
class TestRealmAuditLog(ZulipTestCase):
    def check_role_count_schema(self, role_counts: Dict[str, Any]) -> None:
        for key in [
            ...
            self.assertTrue(
                isinstance(role_counts[RealmAuditLog.ROLE_COUNT_HUMANS][str(key)], int),
            )
    ...
    def test_subscriptions(self) -> None:
        ...
        bulk_remove_subscriptions(
            user, stream, get_client("website"), acting_user=acting_user,
        )
    ...
    def test_get_last_message_id(self) -> None:
        ...
        self.assertEqual(
            get_last_message_id(), Message.objects.latest("id").id,
        )
