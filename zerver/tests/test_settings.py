...
class ChangeSettingsTest(ZulipTestCase):
    ...
    def check_for_toggle_param(self, pattern: str, param: str) -> None:
        ...
        json_result = self.client_post(pattern, {param: ujson.dumps(True)})
        self.assert_json_success(json_result)
        # refetch user_profile object to correctly handle caching
        user_profile = self.example_user("hamlet")
        self.assertEqual(getattr(user_profile, param), True)

        json_result = self.client_post(pattern, {param: ujson.dumps(False)})
        self.assert_json_success(json_result)
        # refetch user_profile object to correctly handle caching
        user_profile = self.example_user("hamlet")
    ...
    def check_for_toggle_param_patch(self, pattern: str, param: str) -> None:
        self.login("hamlet")
        user_profile = self.example_user("hamlet")
        json_result = self.client_patch(pattern, {param: ujson.dumps(True)})
        self.assert_json_success(json_result)
        # refetch user_profile object to correctly handle caching
        user_profile = self.example_user("hamlet")
        self.assertEqual(getattr(user_profile, param), True)

        json_result = self.client_patch(pattern, {param: ujson.dumps(False)})
        self.assert_json_success(json_result)
        # refetch user_profile object to correctly handle caching
        user_profile = self.example_user("hamlet")
        self.assertEqual(getattr(user_profile, param), False)
    ...
    def test_change_notification_sound(self) -> None:
        ...
        json_result = self.client_patch(pattern, {param: ujson.dumps("invalid")})
        self.assert_json_error(json_result, "Invalid notification sound 'invalid'")

        json_result = self.client_patch(pattern, {param: ujson.dumps("ding")})
        self.assert_json_success(json_result)
        ...
        json_result = self.client_patch(pattern, {param: ujson.dumps("zulip")})
    ...
    def test_change_password_ldap_backend(self) -> None:
        ...
        with self.settings(LDAP_APPEND_DOMAIN="zulip.com", AUTH_LDAP_USER_ATTR_MAP=ldap_user_attr_map):
            ...
            result = self.client_patch(
                "/json/settings",
                dict(old_password=self.ldap_password("hamlet"), new_password="ignored"),  # hamlet's password in ldap
            )
            self.assert_json_error(result, "Your Zulip password is managed in LDAP")
