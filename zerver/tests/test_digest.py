...
class TestDigestEmailMessages(ZulipTestCase):
    ...
    def test_active_users_not_enqueued(
        self, mock_django_timezone: mock.MagicMock, mock_enough_traffic: mock.MagicMock,
    ) -> None:
        # Turn on realm digest emails for all realms
        Realm.objects.update(digest_emails_enabled=True)
        cutoff = timezone_now()
    ...
    def test_new_stream_link(self, mock_django_timezone: mock.MagicMock) -> None:
        ...
        mock_django_timezone.return_value = datetime.datetime(
            year=2017, month=11, day=5, tzinfo=datetime.timezone.utc,
        )
        cordelia = self.example_user("cordelia")
        stream_id = create_stream_if_needed(cordelia.realm, "New stream")[0].id
        new_stream = gather_new_streams(cordelia, cutoff)[1]
        expected_html = (
            f"<a href='http://zulip.testserver/#narrow/stream/{stream_id}-New-stream'>New stream</a>"
        )
