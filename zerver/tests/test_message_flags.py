...
class FirstUnreadAnchorTests(ZulipTestCase):
    ...
    def test_visible_messages_use_first_unread_anchor(self) -> None:
        ...
        with mock.patch("zerver.views.message_fetch.get_first_visible_message_id", return_value=new_message_id + 1):
...
class UnreadCountTests(ZulipTestCase):
    ...
    def test_mark_all_topics_unread_with_invalid_stream_name(self) -> None:
        ...
        invalid_stream_id = "12345678"
        result = self.client_post(
            "/json/mark_topic_as_read", {"stream_id": invalid_stream_id, "topic_name": "whatever"},
        )
        self.assert_json_error(result, "Invalid stream id")
...
class PushNotificationMarkReadFlowsTest(ZulipTestCase):
    ...
    def test_track_active_mobile_push_notifications(self, mock_push_notifications: mock.MagicMock) -> None:
        ...
        result = self.client_post(
            "/json/mark_stream_as_read", {"stream_id": str(stream.id), "topic_name": "test_topic"},
        )
...
class GetUnreadMsgsTest(ZulipTestCase):
    ...
    def test_raw_unread_stream(self) -> None:
        ...
        self.mute_stream(
            user_profile=hamlet, stream=get_stream("test here", realm),
        )

        self.mute_topic(
            user_profile=hamlet, stream_name="devel", topic_name="ruby",
        )
        ...
        self.assertEqual(
            set(stream_dict.keys()), all_message_ids,
        )
    ...
    def test_raw_unread_personal(self) -> None:
        ...
        self.assertEqual(
            pm_dict[cordelia_pm_message_ids[0]], dict(sender_id=cordelia.id),
        )
    ...
    def test_raw_unread_personal_from_self(self) -> None:
        ...
        # which is Othello.
        self.assertEqual(
            pm_dict[othello_msg.id], dict(sender_id=othello.id),
        )
        ...
        self.assertEqual(
            pm_dict[cordelia_msg.id], dict(sender_id=cordelia.id),
        )

        # Send a message to ourself.
        hamlet_msg = send_unread_pm(other_user=hamlet)
        apply_unread_message_event(
            user_profile=hamlet, state=raw_unread_data, message=MessageDict.wide_dict(hamlet_msg), flags=[],
        )
        self.assertEqual(
            set(pm_dict.keys()), {othello_msg.id, cordelia_msg.id, hamlet_msg.id},
        )
...
class MessageAccessTests(ZulipTestCase):
    def test_update_invalid_flags(self) -> None:
        ...
        result = self.client_post(
            "/json/messages/flags", {"messages": ujson.dumps([message]), "op": "add", "flag": "is_private"},
        )
