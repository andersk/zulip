...
class MessagePOSTTest(ZulipTestCase):
    ...
    def test_sending_message_as_stream_post_policy_admins(self) -> None:
        ...
        admin_owned_bot = self.create_test_bot(
            short_name="whatever1", full_name="whatever1", user_profile=admin_profile,
        )
    ...
    def test_duplicated_mirrored_huddle(self) -> None:
        ...
        msg = {
            ...
            "content": "Test message",
            "client": "zephyr_mirror",
            "to": ujson.dumps([self.mit_email("espuser"), self.mit_email("starnine")]),
        }
    ...
    def test_unsubscribed_api_super_user(self) -> None:
        ...
        test_with(
            sender_email=cordelia.email, client="test suite", forged=False,
        )

        test_with(
            sender_email="irc_person@zulip.com", client="irc_mirror", forged=True,
        )
...
class StreamMessagesTest(ZulipTestCase):
    ...
    def test_stream_message_unicode(self) -> None:
        ...
        self.assertEqual(
            str(message), "<Message: Denmark / my topic / " "<UserProfile: {} {}>>".format(sender.email, sender.realm),
        )
...
class PersonalMessageSendTest(ZulipTestCase):
    ...
    def test_personal(self) -> None:
        ...
        self.assert_personal(
            sender=self.example_user("hamlet"), receiver=self.example_user("othello"),
        )
...
class ExtractTest(ZulipTestCase):
    def test_extract_stream_indicator(self) -> None:
        ...
        self.assertEqual(
            extract_stream_indicator('["social"]'), "social",
        )

        self.assertEqual(
            extract_stream_indicator("[123]"), 123,
        )
