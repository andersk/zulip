    ...
    try:
        ...
        if not client.event_queue.empty() or dont_block:
            response: Dict[str, Any] = dict(
                events=client.event_queue.contents(), handler_id=handler_id,
            )
...
def get_client_info_for_message_event(
    ...
    for user_data in users:
        ...
        for client in get_client_descriptors_for_user(user_profile_id):
            send_to_clients[client.event_queue.id] = dict(
                client=client, flags=flags, is_sender=is_sender_client(client),
            )
...
def process_message_event(
    ...
    for client_data in send_to_clients.values():
        ...
        extra_data: Optional[Mapping[str, bool]] = extra_user_data.get(
            client.user_profile_id, None,
        )
