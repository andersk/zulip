...
class Command(BaseCommand):
    ...
    def _check_within_range(
        self, key: str, count_func: Callable[[], int], trim_func: Optional[Callable[[str, int], None]] = None,
    ) -> None:
...
than max_api_calls! (trying to trim) %s %s",
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        trim_func: Optional[Callable[[str, int], None]] = lambda key, max_calls: client.ltrim(key, 0, max_calls - 1)
        if not options["trim"]:
            trim_func = None
