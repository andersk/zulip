...
"""
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "email", metavar="<email>", type=str, help="email of user to knight",
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if options["grant"]:
            if (
                user.is_realm_admin
                and options["permission"] == "administer"
                ...
                if options["ack"]:
                    if options["permission"] == "api_super_user":
                        ...
                        do_change_user_role(
                            user, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=None,
                        )
                    print("Done!")
                else:
                    print(
                        "Would have granted {} {} rights for {}".format(
            ...
            if (
                ...
                if options["ack"]:
                    if options["permission"] == "api_super_user":
                        ...
                        do_change_user_role(
                            user, UserProfile.ROLE_MEMBER, acting_user=None,
                        )
                    print("Done!")
                else:
                    print(
                        "Would have removed {}'s {} rights on {}".format(
                            email, options["permission"], user.realm.string_id,
                        ),
                    )
            else:
