...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--list-id", dest="list_id", type=str, help="List ID of the MailChimp mailing list.")
        parser.add_argument(
            "--optin-time",
            dest="optin_time",
            type=str,
    ...
    def handle(self, *args: Any, **options: Optional[str]) -> None:
        ...
        if api_key is None:
            try:
                ...
                raise CommandError(
                    "Please supply a MailChimp API key to --api-key, or add a "
        ...
        if options["list_id"] is None:
            try:
                ...
                raise CommandError(
                    "Please supply a MailChimp List ID to --list-id, or add a "
        ...
        for user in UserProfile.objects.filter(is_bot=False, is_active=True).values("email", "full_name", "realm_id"):
            data = {
