...
def add_request_metadata(report: Dict[str, Any], request: HttpRequest) -> None:
    ...
    try:
        report["data"] = exception_filter.get_post_parameters(request) if request.method == "POST" else request.GET
    except Exception:
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            if settings.STAGING_ERROR_NOTIFICATIONS:
                ...
                queue_json_publish("error_reports", dict(type="server", report=report))
        except Exception:
