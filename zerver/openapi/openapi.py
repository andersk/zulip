...
def get_schema(endpoint: str, method: str, response: str) -> Dict[str, Any]:
    if len(response) == 3 and (
        ...
        response += "_0"
    if len(response) == 3:
        schema = openapi_spec.spec()["paths"][endpoint][method.lower()]["responses"][response]["content"][
            "application/json"
        ]["schema"]
        return schema
    else:
...
def get_openapi_return_values(endpoint: str, method: str, include_url_parameters: bool = True) -> List[Dict[str, Any]]:
    openapi_endpoint = openapi_spec.spec()["paths"][endpoint][method.lower()]
