...
def get_schema(endpoint: str, method: str, response: str) -> Dict[str, Any]:
    if len(response) == 3 and (
        "oneOf"
        in (openapi_spec.spec())["paths"][endpoint][method.lower()]["responses"][
            response
...
def get_openapi_return_values(
    endpoint: str, method: str, include_url_parameters: bool = True,
) -> List[Dict[str, Any]]:
    openapi_endpoint = openapi_spec.spec()["paths"][endpoint][method.lower()]
    response = openapi_endpoint["responses"]["200"]["content"]["application/json"][
        "schema"
    ]
