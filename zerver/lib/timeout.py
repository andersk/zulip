...
def timeout(timeout: float, func: Callable[..., ResultT], *args: Any, **kwargs: Any) -> ResultT:
    ...
    class TimeoutThread(threading.Thread):
        def __init__(self) -> None:
            ...
            self.exc_info: Tuple[Optional[Type[BaseException]], Optional[BaseException], Optional[TracebackType]] = (
                None,
                None,
                None,
            )
        ...
        def raise_async_timeout(self) -> None:
            ...
            result = ctypes.pythonapi.PyThreadState_SetAsyncExc(tid, ctypes.py_object(TimeoutExpired))
            if result > 1:
