...
def verbose_compile(pattern: str) -> Any:
    return re.compile(f"^(.*?){pattern}(.*?)$", re.DOTALL | re.UNICODE | re.VERBOSE)


def normal_compile(pattern: str) -> Any:
    return re.compile(fr"^(.*?){pattern}(.*)$", re.DOTALL | re.UNICODE)
...
def list_of_tlds() -> List[str]:
    ...
    tlds = [tld.lower().strip() for tld in open(tlds_file) if tld not in blacklist and not tld[0].startswith("#")]
