...
def realm_user_count_by_role(realm: Realm) -> Dict[str, Any]:
    human_counts = {
        UserProfile.ROLE_REALM_ADMINISTRATOR: 0,
...
def notify_new_user(user_profile: UserProfile) -> None:
    ...
    if signup_notifications_stream is not None and user_count > 1:
        with override_language(user_profile.realm.default_language):
            message = _(
                "{user} just signed up for Zulip. (total: {user_count})",
            ).format(
...
def get_recipient_info(
    ...
    if user_ids:
        ...
        query = query_for_ids(query=query, user_ids=sorted(list(user_ids)), field="id")
        rows = list(query)
    else:
    ...
    push_notify_user_ids = get_ids_for(lambda r: r["enable_online_push_notifications"])

    # Service bots don't get UserMessage rows.
    um_eligible_user_ids = get_ids_for(lambda r: not is_service_bot(r))
...
def get_service_bot_events(
    ...
    for user_profile_id, bot_type in service_bot_tuples:
        maybe_add_event(
            user_profile_id=user_profile_id, bot_type=bot_type,
        )
...
def do_send_messages(
    messages_maybe_none: Sequence[Optional[MutableMapping[str, Any]]],
    email_gateway: bool = False,
    ...
    for message in messages:
        ...
        if message["message"].is_stream_message():
            ...
            stream_topic: Optional[StreamTopicTarget] = StreamTopicTarget(
                stream_id=stream_id, topic_name=message["message"].topic_name(),
            )
        else:
            stream_topic = None
...
def already_sent_mirrored_message_id(message: Message) -> Optional[int]:
    ...
    messages = filter_by_exact_message_topic(query=query, message=message)
...
def check_send_message(
    ...
    addressee = Addressee.legacy_build(
        sender, message_type_name, message_to, topic_name,
    )
...
def _internal_prep_message(
    realm: Realm, sender: UserProfile, addressee: Addressee, content: str,
...
def internal_prep_stream_message(
    realm: Realm, sender: UserProfile, stream: Stream, topic: str, content: str,
) -> Optional[Dict[str, Any]]:
    """
...
def pick_color(user_profile: UserProfile, subs: Iterable[Subscription]) -> str:
    ...
    if available_colors:
        ...
        return STREAM_ASSIGNMENT_COLORS[
            len(used_colors) % len(STREAM_ASSIGNMENT_COLORS)
        ]
...
def validate_user_access_to_subscribers(
    ...
    validate_user_access_to_subscribers_helper(
        user_profile,
        {"realm_id": stream.realm_id, "invite_only": stream.invite_only},
...
def bulk_get_subscriber_user_ids(
    ...
    recipient_ids = sorted(
        [stream_recipient.recipient_id_for(stream_id) for stream_id in stream_ids],
    )
...
def get_peer_user_ids_for_stream_change(
    stream: Stream, altered_user_ids: Iterable[int], subscribed_user_ids: Iterable[int],
) -> Set[int]:
    """
...
def bulk_add_subscriptions(
    ...
    recipient_ids: List[int] = [
        recipient_id for recipient_id in recipients_map.values()
    ]
    ...
    for stream in streams:
        if not stream.is_public():
            ...
            new_users_ids = [
                user.id
                for user in users
                if (user.id, stream.id) in new_streams
                and user.id not in realm_admin_ids
            ]
    ...
    for stream in streams:
        ...
        new_user_ids = [
            user.id for user in users if (user.id, stream.id) in new_streams
        ]
    ...
    return (
        [(user_profile, stream) for (user_profile, recipient_id, stream) in new_subs]
    ...
    def get_non_subscribed_tups() -> List[Tuple[UserProfile, Stream]]:
        ...
        for user_profile in users:
            ...
            subscribed_stream_ids = {
                stream.id for (sub, stream) in user_sub_stream_info
            }
...
def check_change_bot_full_name(
    ...
    check_bot_name_available(
        realm_id=user_profile.realm_id, full_name=new_full_name,
    )
    do_change_full_name(user_profile, new_full_name, acting_user)
...
def do_change_bot_owner(
    ...
    if previous_owner and not previous_owner.is_realm_admin:
        send_event(
            user_profile.realm,
            dict(type="realm_bot", op="delete", bot=dict(user_id=user_profile.id)),
            {previous_owner.id},
        )
    ...
    if user_profile.is_bot:
        send_event(
            ...
            dict(
                type="realm_bot",
                op="update",
                bot=dict(user_id=user_profile.id, api_key=new_api_key),
            ),
            bot_owner_user_ids(user_profile),
        )
...
def do_create_realm(
    ...
    try:
        ...
        internal_send_stream_message(
            admin_realm, sender, signups_stream, topic, signup_message,
        )
...
def notify_default_streams(realm: Realm) -> None:
    event = dict(
        ...
        default_streams=streams_to_dicts_sorted(
...
def do_add_default_stream(stream: Stream) -> None:
    ...
    stream_id = stream.id
    if not DefaultStream.objects.filter(
...
def do_update_user_presence(
    ...
    defaults = dict(timestamp=log_time, status=status, realm_id=user_profile.realm_id)
...
def do_mark_stream_messages_as_read(
    ...
    if topic_name:
        msgs = filter_by_topic_name_via_message(query=msgs, topic_name=topic_name)

    msgs = msgs.extra(where=[UserMessage.where_unread()])
    ...
    * or both
    ...
    if content is not None:
        ...
        if message.is_stream_message():
            ...
            stream_topic: Optional[StreamTopicTarget] = StreamTopicTarget(
                stream_id=stream_id, topic_name=new_topic_name,
            )
        else:
            stream_topic = None
...
def get_streams_traffic(stream_ids: Set[int]) -> Dict[int, int]:
    ...
    query = StreamCount.objects.filter(
        property=stat.property, end_time__gt=traffic_from,
    )
...
def gather_subscriptions_helper(
    ...
    sub_recipient_ids = [sub["recipient_id"] for sub in sub_dicts]
    stream_recipient = StreamRecipientMap()
    ...
    all_streams = (
        get_active_streams(user_profile.realm)
    ...
    if include_subscribers:
        subscriber_map: Mapping[
            int, Optional[List[int]],
        ] = bulk_get_subscriber_user_ids(
            all_streams, user_profile, streams_subscribed_map, stream_recipient,
        )
    else:
    ...
    return (
        sorted(subscribed, key=lambda x: x["name"]),
...
def email_not_system_bot(email: str) -> None:
    if is_cross_realm_bot_email(email):
        ...
        code = msg
        raise ValidationError(
            msg, code=code, params=dict(deactivated=False),
        )
...
def do_remove_alert_words(
    user_profile: UserProfile, alert_words: Iterable[str],
) -> None:
...
def do_update_user_custom_profile_data_if_changed(
    user_profile: UserProfile, data: List[Dict[str, Union[int, str, List[int]]]],
) -> None:
    with transaction.atomic():
        for field in data:
...
def do_send_user_group_update_event(
    user_group: UserGroup, data: Dict[str, Any],
) -> None:
...
def notify_realm_export(user_profile: UserProfile) -> None:
    ...
    event = dict(
        type="realm_export", exports=get_realm_exports_serialized(user_profile),
    )
    send_event(user_profile.realm, event, [user_profile.id])
