...
def create_missed_message_address(user_profile: UserProfile, message: Message) -> str:
    if settings.EMAIL_GATEWAY_PATTERN == "":
        logger.warning("EMAIL_GATEWAY_PATTERN is an empty string, using " "NOREPLY_EMAIL_ADDRESS in the 'from' field.")
...
def process_stream_message(to: str, message: EmailMessage) -> None:
    ...
    logger.info(
        "Successfully processed email to %s (%s)", stream.name, stream.realm.string_id,
    )
...
def process_missed_message(to: str, message: EmailMessage) -> None:
    ...
    if recipient.type == Recipient.STREAM:
        ...
        internal_send_stream_message(
            user_profile.realm, user_profile, stream, topic, body,
        )
