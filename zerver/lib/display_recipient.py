...
def get_display_recipient_remote_cache(
    ...
    user_profile_list = (
        UserProfile.objects.filter(subscription__recipient_id=recipient_id)
        .order_by("id")
...
def bulk_get_user_profile_by_id(uids: List[int]) -> Dict[int, UserDisplayRecipient]:
    return bulk_cached_fetch(
        ...
        query_function=lambda ids: list(UserProfile.objects.filter(id__in=ids).values(*display_recipient_fields)),
        object_ids=uids,
