...
def get_topic_mutes(user_profile: UserProfile) -> List[Tuple[str, str, float]]:
    ...
    return [
        (row["stream__name"], row["topic_name"], datetime_to_timestamp(row["date_muted"])) for row in rows
    ]
...
def set_topic_mutes(
    ...
    MutedTopic.objects.filter(user_profile=user_profile).delete()
...
def exclude_topic_mutes(
    conditions: List[Selectable], user_profile: UserProfile, stream_id: Optional[int],
) -> List[Selectable]:
    ...
    query = query.values("recipient_id", "topic_name")
    rows = list(query)
