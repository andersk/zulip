...
def apply_event(
    ...
    if event["type"] == "message":
        ...
        if event["op"] == "add":
            ...
            for sub in state["subscriptions"]:
                if sub["stream_id"] == stream_id and user_id not in sub["subscribers"]:
            ...
            user_id = event["user_id"]
            for sub in state["subscriptions"]:
                if sub["stream_id"] == stream_id and user_id in sub["subscribers"]:
        ...
        if event["flag"] == "starred" and "starred_messages" in state:
            ...
            if event["operation"] == "remove":
                state["starred_messages"] = [
                    message for message in state["starred_messages"] if not (message in event["messages"])
                ]
