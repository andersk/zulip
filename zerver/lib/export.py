...
def export_from_config(
    ...
    if config.is_seeded:
        ...
        config.custom_fetch(
            response=response, config=config, context=context,
        )
    ...
    for child_config in config.children:
        export_from_config(
            response=response, config=child_config, context=context,
        )
...
def get_realm_config() -> Config:
    ...
    Config(
        table="zerver_realmemoji", model=RealmEmoji, normal_parent=realm_config, parent_key="realm_id__in",
    )
    ...
    Config(
        table="zerver_client", model=Client, virtual_parent=realm_config, use_all=True,
    )
...
def export_partial_message_files(
    ...
    if public_only:
        messages_we_received = Message.objects.filter(
            sender__in=ids_of_our_possible_senders, recipient__in=recipient_ids_for_us,
        ).order_by("id")
...
def export_avatars_from_local(realm: Realm, local_dir: Path, output_dir: Path) -> None:
    ...
    for user in users:
        ...
        for local_path in glob.glob(wildcard):
            logging.info(
                "Copying avatar file for user %s from %s", user.email, local_path,
            )
...
def export_single_user(user_profile: UserProfile, response: TableData) -> None:
    ...
    export_from_config(
        response=response, config=config, seed_object=user_profile,
    )
...
def export_analytics_tables(realm: Realm, output_dir: Path) -> None:
    ...
    export_from_config(
        response=response, config=config, seed_object=realm,
    )
    write_data_to_file(output_file=export_file, data=response)
