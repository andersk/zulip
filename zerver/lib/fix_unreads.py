...
def fix_unsubscribed(cursor: CursorObj, user_profile: UserProfile) -> None:
    ...
    get_timing(
        "get recipients", find_recipients,
    )
    ...
    def find() -> None:
        ...
        cursor.execute(
            query, {"user_profile_id": user_profile.id, "recipient_ids": tuple(recipient_ids)},
        )
        rows = cursor.fetchall()
        for row in rows:
    ...
    get_timing(
        "finding unread messages for non-active streams", find,
    )
    ...
    get_timing(
        "fixing unread messages for non-active streams", fix,
    )
