...
emoticon_regex = (
    f"(?<![^{terminal_symbols}])(?P<emoticon>(" + ")|(".join(possible_emoticon_regexes) + f"))(?![^{terminal_symbols}])"
)
...
def check_emoji_admin(user_profile: UserProfile, emoji_name: Optional[str] = None) -> None:
    ...
    emoji = RealmEmoji.objects.filter(realm=user_profile.realm, name=emoji_name, deactivated=False).first()
    current_user_is_author = emoji is not None and emoji.author is not None and emoji.author.id == user_profile.id
