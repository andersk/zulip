...
def get_status_dicts_for_rows(
    ...
    for user_key, presence_rows in itertools.groupby(all_rows, get_user_key):
        info = get_user_info(list(presence_rows), mobile_user_ids=mobile_user_ids)
...
def get_status_dict_by_realm(realm_id: int, slim_presence: bool = False) -> Dict[str, Dict[str, Any]]:
    ...
    query = UserPresence.objects.filter(
        realm_id=realm_id, timestamp__gte=two_weeks_ago, user_profile__is_active=True, user_profile__is_bot=False,
    ).values(
        "client__name",
        "status",
        "timestamp",
