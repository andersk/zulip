...
def delete_realm_user_sessions(realm: Realm) -> None:
    realm_user_ids = [
        user_profile.id for user_profile in UserProfile.objects.filter(realm=realm)
    ]
...
def delete_all_deactivated_user_sessions() -> None:
    for session in Session.objects.all():
        ...
        if (
            user_profile_id is None
        ):  # nocoverage  # TODO: Investigate why we lost coverage on this
            continue
        user_profile = get_user_profile_by_id(user_profile_id)
        if not user_profile.is_active or user_profile.realm.deactivated:
            logging.info(
                "Deactivating session for deactivated user %s", user_profile.id,
            )
            delete_session(session)
...
def set_expirable_session_var(
    ...
    expire_at = datetime_to_timestamp(
        timezone_now() + timedelta(seconds=expiry_seconds),
    )
