...
def filter_by_subscription_history(
    ...
    def store_user_message_to_insert(message: Message) -> None:
        message = UserMessage(
            user_profile=user_profile, message_id=message["id"], flags=0,
        )
...
def add_missing_messages(user_profile: UserProfile) -> None:
    ...
    * Create the UserMessage rows.
    ...
    all_stream_subscription_logs: DefaultDict[int, List[RealmAuditLog]] = defaultdict(
        list,
    )
...
def do_soft_deactivate_users(users: List[UserProfile]) -> List[UserProfile]:
    ...
    while True:
        ...
        logging.info(
            "Soft-deactivated batch of %s users; %s remain to process",
            len(user_batch),
            len(users),
        )
...
def do_auto_soft_deactivate_users(
    ...
    users_to_deactivate = get_users_for_soft_deactivation(
        inactive_for_days, filter_kwargs,
    )
...
def get_users_for_soft_deactivation(
    ...
    users_to_deactivate = list(
        UserProfile.objects.filter(id__in=user_ids_to_deactivate),
    )
...
def get_soft_deactivated_users_for_catch_up(filter_kwargs: Any) -> List[UserProfile]:
    users_to_catch_up = UserProfile.objects.select_related().filter(
        long_term_idle=True, is_active=True, is_bot=False, **filter_kwargs,
    )
