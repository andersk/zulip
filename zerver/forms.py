...
DEACTIVATED_ACCOUNT_ERROR = (
    "Your account is no longer active. " + "Please contact your organization administrator to reactivate it."
)
...
class HomepageForm(forms.Form):
    ...
    def clean_email(self) -> str:
        ...
        if not from_multiuse_invite and realm.invite_required:
            raise ValidationError(
                _("Please request an invite for {email} " "from the organization " "administrator.").format(
                    email=email,
                ),
            )
...
def generate_password_reset_url(user_profile: UserProfile, token_generator: PasswordResetTokenGenerator) -> str:
    ...
    endpoint = reverse("django.contrib.auth.views.password_reset_confirm", kwargs=dict(uidb64=uid, token=token))
    ...
    def clean(self) -> Dict[str, Any]:
        ...
        if username is not None and password:
            ...
            try:
                ...
                logging.warning("User %s attempted to password login to nonexistent subdomain %s", username, subdomain)
