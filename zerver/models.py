...
def query_for_ids(query: QuerySet, user_ids: List[int], field: str) -> QuerySet:
    ...
    assert user_ids
    clause = f"{field} IN %s"
    query = query.extra(where=[clause], params=(tuple(user_ids),))
    return query
...
def get_display_recipient(recipient: "Recipient") -> DisplayRecipientT:
    return get_display_recipient_by_id(recipient.id, recipient.type, recipient.type_id)
...
class Realm(models.Model):
    ...
    bot_creation_policy: int = models.PositiveSmallIntegerField(
        default=BOT_CREATION_EVERYONE,
    )
    ...
    VIDEO_CHAT_PROVIDERS = {
        "disabled": {"name": "None", "id": 0},
        "jitsi_meet": {"name": "Jitsi Meet", "id": 1},
    ...
    def currently_used_upload_space_bytes(self) -> int:
        used_space = Attachment.objects.filter(realm=self).aggregate(Sum("size"))[
            "size__sum"
        ]
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    last_reminder: Optional[datetime.datetime] = models.DateTimeField(
    ...
    def profile_data(self) -> ProfileData:
        ...
        user_data = {
            v.field_id: {"value": v.value, "rendered_value": v.rendered_value}
            for v in values
        }
        data: ProfileData = []
...
class PreregistrationUser(models.Model):
    ...
    INVITE_AS = dict(REALM_OWNER=100, REALM_ADMIN=200, MEMBER=400, GUEST_USER=600)
...
def active_non_guest_user_ids(realm_id: int) -> List[int]:
    query = (
        UserProfile.objects.filter(realm_id=realm_id, is_active=True)
