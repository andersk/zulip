...
def build_recipient(type_id: int, recipient_id: int, type: int) -> ZerverFieldsT:
    recipient = Recipient(type_id=type_id, id=recipient_id, type=type)  # stream id
...
def write_avatar_png(avatar_folder: str, realm_id: int, user_id: int, bits: bytes) -> ZerverFieldsT:
    ...
    """
    avatar_hash = user_avatar_path_from_ids(user_profile_id=user_id, realm_id=realm_id)
...
) -> Iterator[Tuple[int, List[ListJobData]]]:
