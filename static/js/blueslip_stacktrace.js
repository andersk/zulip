...
export function clean_path(full_path) {
...
export function clean_function_name(function_name) {
...
async function get_context(location) {
    ...
    return lines.slice(lo_line_num, hi_line_num).map((line, i) => ({
        line_number: lo_line_num + i + 1,
        line,
        focus: lo_line_num + i + 1 === lineNumber,
    }));
}
export async function display_stacktrace(error, stack) {
    const ex = new Error("dummy");
    ex.stack = stack;
    const stackframes = await Promise.all(
        ErrorStackParser.parse(ex).map(async (stack_frame) => {
