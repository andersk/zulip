...
export function subscribe_myself(sub) {
    ...
    add_subscriber(sub.stream_id, user_id);
    sub.subscribed = true;
...
export function unsubscribe_myself(sub) {
    ...
    const user_id = people.my_current_user_id();
    remove_subscriber(sub.stream_id, user_id);
...
export function get_non_default_stream_names() {
    ...
    subs = subs.filter(
        (sub) => !is_default_stream_id(sub.stream_id) && (sub.subscribed || !sub.invite_only),
    );
...
export function update_subscribers_count(sub) {
    const count = sub.subscribers.size;
    sub.subscriber_count = count;
}

export function potential_subscribers(sub) {
    /*
...
export function create_sub_from_server_data(attrs) {
    ...
    set_subscribers(sub, subscriber_user_ids);
...
export function sort_for_stream_settings(stream_ids, order) {
    ...
    function by_subscriber_count(id_a, id_b) {
        const out = get_sub_by_id(id_b).subscribers.size - get_sub_by_id(id_a).subscribers.size;
        if (out === 0) {
...
*/
export const realm_has_notifications_stream = () =>
