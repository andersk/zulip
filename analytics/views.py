...
def realm_summary_table(realm_minutes: Dict[str, float]) -> str:
    ...
    for up in UserProfile.objects.select_related("realm").filter(
        role=UserProfile.ROLE_REALM_ADMINISTRATOR, is_active=True,
    ):
...
def ad_hoc_queries() -> List[Dict[str, str]]:
    ...
    for mobile_type in ["Android", "ZulipiOS"]:
        ...
        """,
        ).format(mobile_type=Literal(mobile_type))
...
def get_user_activity_records_for_email(email: str) -> List[QuerySet]:
    ...
    records = UserActivity.objects.filter(user_profile__delivery_email=email)
