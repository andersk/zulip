...
class Command(BaseCommand):
    ...
    def private_messages(self, realm: Realm, days_ago: int) -> int:
        sent_time_cutoff = timezone_now() - datetime.timedelta(days=days_ago)
        return (
            human_messages.filter(sender__realm=realm, date_sent__gt=sent_time_cutoff)
            .exclude(recipient__type=Recipient.STREAM)
            .exclude(recipient__type=Recipient.HUDDLE)
            .count()
        )
    ...
    def group_private_messages(self, realm: Realm, days_ago: int) -> int:
        ...
        return (
            human_messages.filter(sender__realm=realm, date_sent__gt=sent_time_cutoff)
            .exclude(recipient__type=Recipient.STREAM)
            .exclude(recipient__type=Recipient.PERSONAL)
            .count()
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for realm in realms:
            ...
            streams = (
                ...
                .extra(
                    ...
                    where=[
                        ...
                        "zerver_subscription.active = true",
                    ],
                )
            ...
            num_notifications_enabled = len(
                [x for x in active_users if x.enable_desktop_notifications],
            )
            ...
            # Code block markup
            markup_messages = (
                human_messages.filter(sender__realm=realm, content__contains="~~~")
                .values("sender")
            ...
            notifications = (
                active_user_subs.filter(desktop_notifications=True)
                .values("user_profile")
                .annotate(count=Count("user_profile"))
            )
            print(
