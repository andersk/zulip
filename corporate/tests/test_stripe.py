...
def normalize_fixture_data(
    ...
    for fixture_file in fixture_files_for_function(decorated_function):
        ...
        # Dates
        file_content = re.sub(
            r'(?<="Date": )"(.* GMT)"', '"NORMALIZED DATETIME"', file_content,
        )
        file_content = re.sub(
...
class StripeTestCase(ZulipTestCase):
    def setUp(self, *mocks: Mock) -> None:
        ...
        self.assertEqual(
            UserProfile.objects.filter(realm=realm, is_active=True).count(), 8,
        )

        # Make sure we have active users outside our realm (to make
        # sure relevant queries restrict on realm).
        self.assertEqual(
            UserProfile.objects.exclude(realm=realm).filter(is_active=True).count(), 10,
        )
    ...
    def upgrade(
        ...
        **kwargs: Any,
        ...
        if invoice:  # send_invoice
            params.update({"billing_modality": "send_invoice", "licenses": 123})
    ...
    def local_upgrade(self, *args: Any) -> None:
        ...
        for mocked_function_name in MOCKED_STRIPE_FUNCTION_NAMES:
            upgrade_func = patch(mocked_function_name, return_value=StripeMock())(
                upgrade_func,
            )
        upgrade_func(*args)
...
class StripeTest(StripeTestCase):
    ...
    def test_free_trial_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            self.assert_in_success_response(
                ["Pay annually", "Free Trial", "60 day"], response,
            )
            self.assertNotEqual(user.realm.plan_type, Realm.STANDARD)
            self.assertFalse(Customer.objects.filter(realm=user.realm).exists())
    ...
    def test_billing_page_permissions(self, *mocks: Mock) -> None:
        ...
        response = self.client_get("/billing/")
        self.assert_in_success_response(
            ["You must be an organization administrator"], response,
        )
    ...
    def test_upgrade_by_card_with_outdated_seat_count(self, *mocks: Mock) -> None:
        ...
        self.assertEqual(
            LicenseLedger.objects.first().licenses_at_next_renewal, new_seat_count,
        )
    ...
    def test_upgrade_license_counts(self) -> None:
        ...
        check_min_licenses_error(
            False, None, self.seat_count, {"license_management": "manual"},
        )
    ...
    def test_switch_from_monthly_plan_to_annual_plan_for_automatic_license_management(
        ...
        self.assertEqual(
            annual_plan.invoicing_status, CustomerPlan.INITIAL_INVOICE_TO_BE_SENT,
        )
        ...
        annual_plan_invoice_items = [
            invoice_item for invoice_item in invoices[0].get("lines")
        ]
        ...
        monthly_plan_invoice_items = [
            invoice_item for invoice_item in invoices[1].get("lines")
        ]
    ...
    def test_switch_from_monthly_plan_to_annual_plan_for_manual_license_management(
        ...
        with patch("corporate.lib.stripe.timezone_now", return_value=self.now):
            self.upgrade(
                schedule="monthly", license_management="manual", licenses=num_licenses,
            )
        monthly_plan = get_current_plan_by_realm(user.realm)
        assert monthly_plan is not None
...
class InvoiceTest(StripeTestCase):
    ...
    def test_invoice_plan(self, *mocks: Mock) -> None:
        ...
        # Decrease
        with patch(
            "corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count,
        ):
            update_license_ledger_if_needed(
                get_realm("zulip"), self.now + timedelta(days=200),
            )
        # Increase, but not past high watermark
        with patch(
