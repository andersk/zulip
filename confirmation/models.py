...
def create_confirmation_link(obj: ContentType, confirmation_type: int, url_args: Mapping[str, str] = {}) -> str:
    key = generate_key()
    realm = None
    ...
    Confirmation.objects.create(
        content_object=obj, date_sent=timezone_now(), confirmation_key=key, realm=realm, type=confirmation_type,
    )
...
_properties = {
    ...
    Confirmation.UNSUBSCRIBE: ConfirmationType(
        "zerver.views.unsubscribe.email_unsubscribe", validity_in_days=1000000,
    ),  # should never expire
...
def generate_realm_creation_url(by_admin: bool = False) -> str:
    ...
    return urljoin(settings.ROOT_DOMAIN_URI, reverse("zerver.views.create_realm", kwargs={"creation_key": key}))
